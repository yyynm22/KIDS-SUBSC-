...
fetchOrderHistory() {
    ...
    // ユーザーIDに基づく注文履歴を取得するAPIエンドポイント
    axios.get('https://m3h-yuunaminagawa.azurewebsites.net/api/SELECT4', {
        params: { user_id: userId } // ユーザーIDをパラメーターとして追加
    })
    .then(select4Response => {
        const orderList = select4Response.data.List;

        // 各order_idに対してSELECT6を呼び出す
        const orderIds = [...new Set(orderList.map(order => order.order_id))]; // 重複を除いた注文IDのリスト

        const select6Promises = orderIds.map(order_id =>
            axios.get('https://m3h-yuunaminagawa.azurewebsites.net/api/SELECT6', {
                params: { order_id } // 注文IDをパラメーターとして追加
            })
        );

        return Promise.all(select6Promises).then(select6Responses => {
            const orderDetails = select6Responses.map((response, index) => {
